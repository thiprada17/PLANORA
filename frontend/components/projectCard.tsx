...
export default function ProjectCard({
  ...
  return (
    ...
    >
      ...
      >
        <View
          className="self-start bg-white px-3 rounded-full mb-2 flex-row items-center"
          style={{
            shadowColor: "#bfbfbf",
            shadowOpacity: 0.3,
            shadowRadius: 3.5,
        ...
        <Text
          className="font-KanitBold text-2xl text-black mb-1"
          numberOfLines={1}
        >
          {project_name}
        </Text>
        ...
        <View className="flex-row items-center justify-between mt-auto">
          <View className="flex-row">
            {members.slice(0, 4).map((m, i) => (
              <View
                key={m.id}
                className={`w-8 h-8 rounded-full bg-white border border-black/30 items-center justify-center ${
                  i !== 0 ? "-ml-4" : ""
                }`}
              >
                <Image

...
export default function HomePage() {
  ...
  >
    ...
    <Text className={`text-[9px] ${statusValue === "ALL" ? "text-gray-400" : "text-black"}`}>
      {statusValue}
    </Text>
  ...
  {showStatusCard && (
    <View className="absolute left-0 top-[38px] z-50 w-[150px] bg-white rounded-2xl py-2 border border-gray-300 shadow">
      {statusItems.map(item => (
        <Pressable
          key={item.value}
          onPress={() => { setStatusValue(item.value); setShowStatusCard(false); }}
          className="px-4 py-2 flex-row justify-between items-center"
        >
          <Text className="text-xs">{item.label}</Text>
          {statusValue === item.value && <Text className="text-xs">✓</Text>}
        </Pressable>
      ))}
    </View>
  )}
  ...
  >
    <Image
      source={icons.filter}
      className="w-4 h-4"
      style={subjectFilter !== "ALL" ? { tintColor: "#fff" } : undefined}
    />
  </Pressable>
  ...
  {showSubjectCard && (
    <View className="absolute right-0 top-[38px] z-50 w-[150px] bg-white rounded-2xl py-2 border border-gray-300 shadow">
      {subjectItems.map(item => (
        <Pressable
          key={item.value}
          onPress={() => { setSubjectFilter(item.value); setShowSubjectCard(false); }}
          className="px-4 py-2 flex-row justify-between items-center"
        >
          <Text className="text-xs">{item.label}</Text>
          {subjectFilter === item.value && <Text className="text-xs">✓</Text>}
        </Pressable>
      ))}
    </View>
     )}
